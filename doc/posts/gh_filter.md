...
&= \tilde{x}_{t+1} + g*(\tilde{x}_{t+1}-z_{t+1}) \\
\\
估计值 &= 模型预测 + g*(测量值-模型预测)
\end{align}
$$
...
> 现在你应该理解为什么我说滤波的核心就是 预测 (Precition) 和 测量 (Measurement) 了，不同的滤波算法其实就是在权衡到底应该相信谁：有的人相信理论，有的人相信现实，有的人动态地去相信，例如 Kalman 滤波器。
...
- h=0，说明我们认为系统速度不变，也就是水温不会变化。

于是，$g-h$ 不管传感器测量结果是多少，都估计水温不变，就画出了一条直线。

> 这里你可能会问，那这个直线的数值是怎么确定的呢？$g-h$ 滤波器需要你指定一个初始值 $x_0$ 也就是最初的水温。如果你知道初始值当然最好，不知道的话 $x_0=0$ 就行了，后面 $g-h$ 滤波器会根据测量值，逐渐逼近真实水温的。
...
前面我反复强调，滤波是一个动态过程，所以这里我用动画来展示滤波的效果。
...
> 最后，还想再强调一次，滤波器是一个动态的过程，希望动态地过滤掉传感器的噪声。
>
> 另外，滤波器最核心的2个概念：预测 (Prediction) 和 测量 (Measurement)，同样的思想在控制领域和强化学习也广为流传。
